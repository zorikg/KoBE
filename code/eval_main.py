...
def genarate_results_table_for_paper(submitted_qe_and_bleu_and_us_df_correl_df_raw, lang_pairs_strings):
    df = submitted_qe_and_bleu_and_us_df_correl_df_raw[lang_pairs_strings].drop(['entity_recall_metric']).rename(index={'entity_recall_qe': 'MKBE'}).dropna(how='all')
    df.fillna(value='--', inplace=True)
    return df

def main(base_path):
    lang_pairs_strings = ['de-en', 'fi-en', 'gu-en', 'kk-en', 'lt-en', 'ru-en', 'zh-en', 'en-cs', 'en-de', 'en-fi', 'en-gu', 'en-kk', 'en-lt', 'en-ru', 'en-zh', 'de-cs', 'de-fr', 'fr-de']

    all_annotations = read_all_annotations(os.path.join(base_path, 'annotations/wmt19-submitted-data/newstest2019'), lang_pairs_strings)

    scores = {'entity_recall_qe': defaultdict(dict), 'entity_recall_metric': defaultdict(dict)}
    for lang_pair in all_annotations.keys():
        scores['entity_recall_qe'][lang_pair], scores['entity_recall_metric'][lang_pair] = defaultdict(dict), defaultdict(dict)
        ...
        for sys_name in all_system_names:
            qe_counters = match_ids(all_annotations[lang_pair]['src']['annotated_sentence'], all_annotations[lang_pair][sys_name]['annotated_sentence'])
            metric_counters = match_ids(all_annotations[lang_pair]['ref']['annotated_sentence'], all_annotations[lang_pair][sys_name]['annotated_sentence'])
            scores['entity_recall_qe'][lang_pair][sys_name] = calc_percentage(qe_counters['src_count'], qe_counters['match_count']) * entity_count_penalty(qe_counters['src_count'], qe_counters['cnd_count'])
            scores['entity_recall_metric'][lang_pair][sys_name] = calc_percentage(metric_counters['src_count'], metric_counters['match_count']) * entity_count_penalty(metric_counters['src_count'], metric_counters['cnd_count'])

    all_results_df = pd.read_csv(open(os.path.join(base_path, 'wmt19_metric_task_results/sys-level_scores_metrics.csv')), sep=',')
    del all_results_df['Unnamed: 0']
    submitted_qe_and_bleu_df = all_results_df[['lp', 'DA', 'system', 'BLEU', 'ibm1-morpheme', 'ibm1-pos4gram', 'LASIM', 'LP', 'UNI', 'UNI+', 'USFD', 'USFD-TL', 'YiSi-2', 'YiSi-2_srl']].loc[all_results_df['lp'].isin(set(lang_pairs_strings))]
    ...
    submitted_qe_and_bleu_and_us_df = pd.merge(submitted_qe_and_bleu_df, scores_df, how='outer',left_on=['lp', 'system'], right_on=['lp', 'system'])
    submitted_qe_and_bleu_and_us_df = submitted_qe_and_bleu_and_us_df[(submitted_qe_and_bleu_and_us_df['system'] != 'online-B.0') | (submitted_qe_and_bleu_and_us_df['lp'] != 'gu-en')]  # Scores for 'online-B.0' in 'gu-en' are missing
    submitted_qe_and_bleu_and_us_df_correl_df_raw = get_correllations(submitted_qe_and_bleu_and_us_df, lang_pairs_strings)
